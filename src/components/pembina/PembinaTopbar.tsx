...
interface PembinaTopbarProps {
  setIsOpen: (isOpen: boolean) => void;
  isCollapsed?: boolean;
  setIsCollapsed?: (collapsed: boolean) => void;
}

export default function PembinaTopbar({ setIsOpen, isCollapsed, setIsCollapsed }: PembinaTopbarProps) {
  return (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 cursor-pointer p-1 pr-2 rounded-full hover:bg-slate-50 transition-colors">
          ...
          <div className="hidden md:block text-sm">
            <p className="font-semibold text-slate-700 leading-none">Ahmad Sutoyo</p>
            <p className="text-slate-500 text-xs mt-1">Pembina OSIS</p>
          </div>
        </div>
      </div>
    </header>
