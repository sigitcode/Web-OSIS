...
export default function BeritaSection({ beritaList }: BeritaSectionProps) {
  ...
  if (!beritaList || beritaList.length === 0) return null;

  // We'll treat the first item as featured if none are explicitly featured
  const featured = beritaList.find((b) => b.featured) || beritaList[0];
  const others = beritaList.filter((b) => b.id !== featured.id).slice(0, 3);
    ...
    <section id="berita" className="py-24 section-alt relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          ...
          {featured && (
            <AnimateOnScroll>
              <Link href={`/berita/${featured.id}`} className="block h-full">
                <div className="group glass-card rounded-2xl overflow-hidden h-full flex flex-col hover:-translate-y-1 hover:shadow-xl transition-all duration-300 cursor-pointer">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <p className="text-slate-500 leading-relaxed flex-grow line-clamp-3">
                      {featured.ringkasan}
                    </p>
          ...
          <div className="flex flex-col gap-4">
              ...
              <AnimateOnScroll key={item.id} delay={index * 100}>
                <Link href={`/berita/${item.id}`} className="block">
                  <div className="group glass-card rounded-2xl p-5 flex gap-5 hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 cursor-pointer">
                    ...
                    <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-primary-500 to-primary-700 flex-shrink-0 flex items-center justify-center overflow-hidden relative">
                      {item.gambar ? (
                        <img 
                          src={item.gambar} 
                          alt={item.judul}
            ...
            {beritaList.length > 4 && (
              <AnimateOnScroll delay={400}>
                <Link href="/berita" className="w-full glass-card rounded-2xl p-4 text-center font-semibold text-primary-600 hover:text-primary-700 hover:bg-primary-50/50 transition-all duration-300 group flex items-center justify-center gap-2">
                  Lihat Semua Berita
